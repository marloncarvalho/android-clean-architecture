...
class UserRemoteConverter @Inject constructor() : Converter<UserResponse, User> {
    ...
    override fun revert(output: User): UserResponse =
            UserResponse(output.id!!, output.name, output.login)

    override fun revert(listOutput: List<User>): List<UserResponse> = listOutput.map { revert(it) }

    override fun convert(listInput: List<UserResponse>): List<User> = listInput.map { convert(it) }
